                                        ...
                                        @if(auth()->user()->Can('trả lời bình luận') || auth()->user()->Can('xóa bình luận'))
                                            <td class="text-center">
                                                @can('trả lời bình luận')
                                                    <a href="#" class="btn btn-small btn-primary btn-comment-reply">Trả
                                                        lời</a>
                                                @endcan
                                                @can('xóa bình luận')
                                                    <a href="#"
                                                       class="btn btn-small btn-danger btn-comment-delete">Xóa</a>
                                                @endcan
                                            </td>
                                        @endif
                                    </tr>
                                        ...
                                        @if (id_parent($child->parent_id, $comments, 'parent_id', 'id') == $comm->id)
                                            <tr data-id="{{ $child->id }}" class="sub-border-dashed @if ($comments->where('parent_id', '>', 0)->count() == $key +
                                                ...
                                                @if(auth()->user()->Can('trả lời bình luận') || auth()->user()->Can('xóa bình luận'))
                                                    <td class="text-center">
                                                        @can('trả lời bình luận')
                                                            <a href="#" class="text-primary btn-comment-reply">Trả
                                                                lời</a>
                                                        @endcan|
                                                        @can('xóa bình luận')
                                                            <a href="#"
