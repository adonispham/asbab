                ...
                <div class="col-lg-12">
                    <ul class="breadcrumb">
                        <li class="breadcumb-item"><a href="{{ route('admin') }}"><i class="fa fa-home"></i> Trang
                                chủ</a></li>
                        <li class="breadcumb-item"><a href="{{ route('admin.news.index') }}">Tin tức</a></li>
                        <li class="active">{{ $new->title }}</li>
                    </ul>
                </div>
            </div>
            <section class="form-admin">
                <form action="{{ route('admin.news.update', ['id' => $new->id]) }}" method="post" class="row"
                    ...
                    <div class="col-lg-9">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="abstract" rows="5"
                                      class="form-control @error('abstract') is-invalid @enderror">{{ $new->abstract }}</textarea>
                            @error('abstract')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
